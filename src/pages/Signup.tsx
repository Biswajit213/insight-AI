...
import { storeSignupData, storeLoginData } from '../services/authApi';
...
export default function Signup() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    // Store signup data into Supabase / PostgreSQL database
  ...
  // Called when user submits the GoogleEmailDialog with their real Gmail & name
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-[#0f172a] flex-col justify-between p-12 relative overflow-hidden border-r border-slate-800">
        ...
        <div className="relative space-y-4">
          ...
          <p className="text-sm text-slate-400 max-w-md">
            Join thousands of data-driven teams using InsightAI to clean datasets, discover anomalies, and forecast metrics.
          </p>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-[#0b1120]">
        ...
        >
          ...
          <div className="relative text-center">
            ...
            <span className="relative bg-[#0b1120] px-3 text-[10px] uppercase font-bold text-slate-500 tracking-wider">or sign up with email</span>
          </div>

          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1" htmlFor="name">Full Name</label>
              <input
                id="name"
                type="text"
                {...register('name')}
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1" htmlFor="password">Password</label>
              <div className="relative">
                <input
                  id="password"
